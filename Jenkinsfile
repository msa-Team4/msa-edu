        ...
        stage('Deploy and Service to Kubernetes') {
            steps {
                script {
                    ...
                    sh "kubectl -n ${NAMESPACE} delete deployments springboot-team4"
        ...
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/demo-app-team4-deployment demo-app-team4=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
